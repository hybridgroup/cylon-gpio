...
RGBLed.prototype.setRGB = function(hex, callback) {
  ...
  this.isHigh = true;
  this.connection.pwmWrite(this.redPin, this._negateOnCathode(val.r) / 256.0);
  this.connection.pwmWrite(this.greenPin, this._negateOnCathode(val.g) / 256.0);
  this.connection.pwmWrite(this.bluePin, this._negateOnCathode(val.b) / 256.0);
